 ...
 *
 * Simulates EBS Market (interbank FX) with tighter spreads.
...
const FX_MIDS = { 'EUR/USD': 1.0850, 'USD/JPY': 154.50, 'GBP/USD': 1.2640 };
const FX_SPREAD_BPS = 1;
...
class EBSAdapter extends EventEmitter {
  ...
  _emitBBO(venueSymbol, baseMid) {
    ...
    const event = {
      ...
      bidPrice, bidSize: 2_000_000 + Math.floor(Math.random() * 8_000_000), bidOrderCount: 0,
      askPrice, askSize: 2_000_000 + Math.floor(Math.random() * 8_000_000), askOrderCount: 0,
...
EBSAdapter.prototype.submitOrder = async function(order, creds) {
...
EBSAdapter.prototype.cancelOrder = async function() { return require('./orderInterface').orderResponse({ venueOrderId: null, clientOrderId: null, status: 'ACKNOWLEDGED' }); };
EBSAdapter.prototype.amendOrder = async function() { throw new Error('EBS FIX stub does not support amend'); };
EBSAdapter.prototype.getOrderStatus = async function() { throw new Error('EBS FIX stub does not support order status query'); };
EBSAdapter.mapSymbol = function(c) { return c; };
