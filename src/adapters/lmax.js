 ...
 *
 * Simulates L1 BBO at 500ms intervals with randomised FX prices.
...
const FX_MIDS = { 'EUR/USD': 1.0850, 'GBP/USD': 1.2640, 'USD/JPY': 154.50, 'EUR/GBP': 0.8585 };
const FX_SPREAD_BPS = 2;
...
class LMAXAdapter extends EventEmitter {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const fill = {
          ...
          fillTs: now, receivedTs: now, commission: 0,
          commissionAsset: symbol.slice(3, 6) || 'USD', slippageBps: 0, arrivalMid: mid,
        };
        this.emit('fill', fill);
  ...
  _emitBBO(venueSymbol, baseMid) {
    ...
    const jitter     = (Math.random() - 0.5) * 0.0002 * baseMid;
    ...
    const event = {
      ...
      bidPrice, bidSize: 1_000_000 + Math.floor(Math.random() * 4_000_000), bidOrderCount: 0,
      askPrice, askSize: 1_000_000 + Math.floor(Math.random() * 4_000_000), askOrderCount: 0,
...
// Standard order interface — wraps existing sendOrder into normalised shape
LMAXAdapter.prototype.submitOrder = async function(order, creds) {
...
LMAXAdapter.prototype.cancelOrder = async function() { return require('./orderInterface').orderResponse({ venueOrderId: null, clientOrderId: null, status: 'ACKNOWLEDGED' }); };
LMAXAdapter.prototype.amendOrder = async function() { throw new Error('LMAX FIX stub does not support amend'); };
LMAXAdapter.prototype.getOrderStatus = async function() { throw new Error('LMAX FIX stub does not support order status query'); };
LMAXAdapter.mapSymbol = function(c) { return c; };
